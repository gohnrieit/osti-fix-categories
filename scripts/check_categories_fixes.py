...
def check_tree(root: pathlib.Path) -> None:
    ...
    dspthread = root / "source/dsp56300/source/dsp56kEmu/dspthread.cpp"
    processor = root / "source/jucePluginLib/processor.cpp"
    ...
    must_contain(base, "/bigobj")
    must_contain(base, "DEFINED ENV{CI}")
    must_contain(dspthread, "#include <chrono>")
    must_contain(dspthread, "high_resolution_clock")
    must_contain(processor, "#include <chrono>")
    must_contain(processor, "system_clock")
