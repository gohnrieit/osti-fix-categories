...
//			return false;
	...
	bool SearchRequest::isValid() const
	{
		return !name.empty() || !tags.empty() || sourceNode || patch
			...
			|| !anyTagOfType.empty() || !noTagOfType.empty();
	}

	bool SearchRequest::operator==(const SearchRequest& _r) const
	{
		// anyTagOfType / noTagOfType must be compared: Categories uses
		// anyTagOfType (patches that have a category tag). Omitting them made
		// parent-filter updates look identical, so the tree count went stale.
		return name == _r.name && tags == _r.tags && sourceNode == _r.sourceNode && patch == _r.patch && sourceType == _r.sourceType
			&& anyTagOfType == _r.anyTagOfType && noTagOfType == _r.noTagOfType;
	}
}
