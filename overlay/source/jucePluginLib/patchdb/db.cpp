...
namespace pluginLib::patchDB
{
	...
	bool DB::loadFile(DataList& _results, const std::string& _file)
	{
		...
		// Virus TI backups / large .mid/.syx dumps can be tens or hundreds of MB.
		// The old 8 MB cap silently skipped those files, so Patch Manager
		// Categories (N) never counted the presets inside them. Still skip empty
		// files and obvious multi-gigabyte junk; parseFileData rejects garbage.
		constexpr size_t kMaxPatchFileBytes = static_cast<size_t>(512) * 1024 * 1024;
		if (!size || size >= kMaxPatchFileBytes)
			return false;
