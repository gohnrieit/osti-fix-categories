...
namespace jucePluginEditorLib::patchManager
{
	...
	void TreeItem::paintItem(juce::Graphics& _g, const int _width, const int _height)
	{
		...
		const auto align = style ? style->getAlign() : juce::Justification(juce::Justification::centredLeft);
		const juce::String title = juce::String::fromUTF8(m_title.c_str());

		juce::String countText;
		if (m_count == g_unknownCount)
			countText = "(?)";
		else if (m_count != g_invalidCount)
			countText = "(" + juce::String(static_cast<juce::int64>(m_count)) + ")";

		// Draw the count as a right-aligned suffix so large Categories (N)
		// values stay visible when the tree column is narrow. Title ellipsizes.
		if (countText.isEmpty())
		{
			_g.drawText(title, 0, 0, _width, _height, align, true);
		}
		else
		{
			const int countW = juce::jmax(_g.getCurrentFont().getStringWidth(countText) + 6, 28);
			const int titleW = juce::jmax(0, _width - countW);
			_g.drawText(title, 0, 0, titleW, _height, align, true);
			_g.drawText(countText, titleW, 0, countW, _height, juce::Justification::centredRight, false);
		}
